...
./scripts/setup-dev-cert.sh       # One-time: create self-signed "PsstWhisperOS Dev" cert
...
./build-app.sh                    # Builds to dist/PsstWhisperOS.app + dist/PsstWhisperOS-1.0.0.dmg

# Raw swift build
swift build -c debug              # Debug binary to .build/debug/PsstWhisperOS
swift build -c release            # Release binary to .build/release/PsstWhisperOS
...
./scripts/reset-tcc.sh --all      # Reset ALL TCC categories for com.anthonyjohn17.psstwhisperos
```
...
**PsstWhisperOS** is a macOS menu bar speech-to-text app. It captures microphone audio, transcribes via WhisperKit (on-device), formats the text, and pastes it into the frontmost app.
...
Bundle ID: `com.anthonyjohn17.psstwhisperos` | Platform: macOS 14+
